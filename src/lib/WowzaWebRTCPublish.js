...
const set = async (props) =>
{
  ...
  if (props.trace != null)
    newState['trace'] = props.trace;
  else newState['trace'] = false;
...
const getUserMedia = (mediaKind) =>
{
  ...
  return new Promise((resolve,reject) =>
  {
    logMsg('WowzaWebRTCPublish.getUserMedia')
    ...
    async function getUserMediaSuccess(stream)
    {
      ...
      {
        logMsg('getUserMediaSuccess: error connecting stream to videoElementPublish, trying createObjectURL');
        logMsg(error);
      ...
      {
        logMsg('getUserMediaSuccess: error creating audio meter');
        logMsg(error2);
      }
...
const getDevices = () =>
{
  return new Promise((resolve,reject) =>
  {
    logMsg('WowzaWebRTCPublish.getDevices');
    navigator.mediaDevices.enumerateDevices().then((devices) =>
    {
      logMsg(JSON.stringify(devices));
      ...
      (e) => {
        logMsg("unable to detect AV devices: " + e);
        reject(e);
      }
    );
  });
...
function setAudioEnabled(enabled) {
  logMsg('WowzaWebRTC.setAudioEnabled:' + enabled);
  setEnabled("audio",enabled);
  setState({audioEnabled:enabled});
}

function setVideoEnabled(enabled) {
  logMsg('WowzaWebRTC.setVideoEnabled:' + enabled);
...
const setCamera = (id) =>
{
  logMsg("WowzaWebRTC.setCamera: " + id);
  if (id === 'screen')
  {
    getDisplayStream()
    .then((stream) => {
...
{
  logMsg("WowzaWebRTC.setMicrophone: " + id);
  let constraints = {...state.constraints};
...
const stop = () =>
{
  logMsg('WowzaWebRTC.stop()');
  WowzaPeerConnectionPublish.stop();
}

const errorHandler = (error) =>
{
  logMsg('WowzaWebRTC ERROR:');
  logMsg(error);
  if (error.message == null)
  {
    if (error.target != null)
    {
      logMsg('typeof error.target: ' + typeof error.target);
    }
  }
  let newError = {...error}
